 ...
 public function delete_brand(){
	...
	public function edit_brand(){
		if ($this->checkLogin('A') == ''){
			...
			$id = $this->uri->segment(4,0);
			$condition = array('id' => $id);
			$this->data['brand_details'] = $this->vehicle_model->get_all_details(VEHICLE_MODELS,array('id'=>$id)); 
			//echo '<pre>'; print_r($this->data['brand_details']->result()); die;
			if ($this->data['brand_details']->num_rows() == 1){
				$this->load->view('admin/vehicles-models/edit_brand',$this->data);
			}else {
				redirect('admin');
			}
		}
	...
	public function editBrand(){
		if ($this->checkLogin('A') == ''){
			...
			$checkDup = $this->vehicle_model->get_all_details(VEHICLE_MODELS,array('id !='=>$aid,'name'=>$name));
			//echo "<pre>"; print_r($checkDup->result());die;
			if($checkDup->num_rows()>0){
				
				$this->setErrorMessage('error','Already exists');
			    redirect('admin/vehicle_models/edit_brand/'.$aid);
			}else{
			$excludeArr = array("status","model_image","id");
			//$config['encrypt_name'] = TRUE;
			$config['overwrite'] = FALSE;
			$config['allowed_types'] = 'jpg|jpeg|gif|png|bmp';
			$config['max_size'] = 2000;
